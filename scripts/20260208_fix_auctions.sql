...
BEGIN
  ...
  SELECT booms INTO v_user_booms FROM public.users WHERE id = auth.uid();
  
  IF v_user_booms IS NULL OR NOT (v_user_booms ? p_boom_name) THEN
  ...
  IF v_boom_qty < 1 THEN
    RAISE EXCEPTION 'You do not own enough of this boom';
  END IF;
  ...
  ELSE
    v_user_booms := jsonb_set(v_user_booms, array[p_boom_name], (v_boom_qty - 1)::text::jsonb);
  END IF;
  
  UPDATE public.users SET booms = v_user_booms WHERE id = auth.uid();
  ...
  SELECT
    p_boom_name,
    auth.uid(),
    (SELECT username FROM public.users WHERE id = auth.uid()::text),
    p_starting_bid,
    v_ends_at,
    'active'
